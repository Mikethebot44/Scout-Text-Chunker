...
__export(src_exports, {
  ...
  LocalModelEmbedder: () => LocalModelEmbedder,
...
var FixedChunker = class {
  ...
  async chunk(text) {
    ...
    while (startIndex < tokens.length) {
      ...
      chunks.push({ id: (0, import_uuid.v4)(), text: chunkText, start, end });
...
var RecursiveChunker = class {
  ...
  async chunk(text) {
    ...
    for (const paragraph of paragraphs) {
      if (this.tokenCount(paragraph.sentence) <= this.chunkSize) {
        chunks.push({ id: (0, import_uuid2.v4)(), text: paragraph.sentence, start: paragraph.start, end: paragraph.end });
      } else {
  ...
  splitParagraph(paragraph, offset) {
    ...
    for (const sentence of sentences) {
      ...
      if (this.tokenCount(tentative) > this.chunkSize && current.length) {
        ...
        chunks.push({ id: (0, import_uuid2.v4)(), text, start: chunkStart, end: chunkEnd });
  ...
  splitTokens(text, offset) {
    ...
    for (let i = 0; i < tokens.length; i += this.chunkSize) {
      ...
      chunks.push({ id: (0, import_uuid2.v4)(), text: chunkText, start, end });
    }
    return chunks;
  }
...
var SemanticChunker = class {
  ...
  async chunk(text) {
    ...
    for (const index of boundaryIndexes) {
      ...
      chunks.push({ id: (0, import_uuid3.v4)(), text: chunkText, start: startSentence.start, end: endSentence.end });
      lastIndex = index + 1;
    }
  ...
  async chunk(text) {
    ...
    for (const paragraph of paragraphs) {
      ...
      if (tokens.length <= this.minChunkSize) {
        chunks.push({ id: (0, import_uuid4.v4)(), text: paragraph.sentence, start: paragraph.start, end: paragraph.end });
      } else {
  ...
  async chunk(text) {
    ...
    for (let i = 1; i < assignments.length; i++) {
      if (assignments[i] !== currentCluster) {
        ...
        chunks.push({ id: (0, import_uuid5.v4)(), text: chunkText2, start: startSentence2.start, end: endSentence2.end, metadata: { cluster: currentCluster } });
        startIndex = i;
    ...
    chunks.push({ id: (0, import_uuid5.v4)(), text: chunkText, start: startSentence.start, end: endSentence.end, metadata: { cluster: currentCluster } });
    return chunks;
  }
...
var SlidingWindowChunker = class {
  ...
  async chunk(text) {
    ...
    for (let start = 0; start < tokens.length; start += stride) {
      ...
      chunks.push({ id: (0, import_uuid6.v4)(), text: chunkText, start: boundaryAdjusted.start, end: boundaryAdjusted.end });
